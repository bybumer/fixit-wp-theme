...
function fixit_handle_contact() {
	...
	if ( ! isset( $_POST['fixit_contact_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['fixit_contact_nonce'] ) ), 'fixit_contact' ) ) {
		wp_safe_redirect( add_query_arg( 'fixit_msg', 'error', $redirect ) . '#elaqe-form' );
		exit;
	}

	// Bot tələsi (honeypot) — doldurulubsa səssizcə uğurlu kimi göstər.
	if ( ! empty( $_POST['fixit_website'] ) ) {
		wp_safe_redirect( add_query_arg( 'fixit_msg', 'ok', $redirect ) . '#elaqe-form' );
		exit;
	}
	...
	if ( '' === $name || ! is_email( $email ) || '' === $message ) {
		wp_safe_redirect( add_query_arg( 'fixit_msg', 'invalid', $redirect ) . '#elaqe-form' );
		exit;
	}
	...
	$subject = sprintf( '[fixit.az] Yeni müraciət — %s', $name );
	$body    = "Saytdan yeni müraciət daxil oldu:\n\n"
		. "Ad: {$name}\n"
		. "E-poçt: {$email}\n"
		. "Telefon: {$phone}\n"
		. "Mövzu: {$topic}\n\n"
